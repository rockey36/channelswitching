    ...
    MacDataDot11* dot11)
{
    ...
    clocktype agingTime = getSimTime(node) - DOT11s_QUEUE_AGING_TIME;
    if (agingTime < 0)
    {
        return;
    }
    ...
    MacDataDot11* dot11)
{
    ...
    clocktype agingTime = getSimTime(node) - DOT11s_ASSOC_ACTIVE_TIMEOUT;
    if (agingTime < 0)
    {
        return;
    }
    ...
    MacDataDot11* dot11)
{
    ...
    clocktype agingTime =
        getSimTime(node) - mp->portalTimeout;
    if (agingTime < 0)
    {
        return;
    }
    ...
    Message* msg)
{
    ...
    dataSeenItem->insertTime = getSimTime(node);
    ...
    MacDataDot11* dot11)
{
    ...
    clocktype agingTime = getSimTime(node) - DOT11s_DATA_SEEN_AGING_TIME;
    if (agingTime < 0)
    {
        return;
    }
    ...
    *isProcessed = FALSE;
        ...
        && (mp->pathMetric == pathProfile.pathMetric))
    {
        ...
        if (neighborItem == NULL)
        {
            ...
            neighborItem->firstBeaconTime = getSimTime(node);
        ...
        neighborItem->beaconInterval = beaconFrame->beaconInterval;
        neighborItem->lastBeaconTime = getSimTime(node);
        ...
        if (mp->state == DOT11s_S_INIT_START)
        {
                ...
                MacDot11ClocktypeToTUs(dot11->beaconInterval))
            {
                ...
                {
                    dot11->beaconTime += DOT11_TIME_UNIT;
                    ...
                    MacDot11StationStartTimerOfGivenType(
                        node, dot11,
                        dot11->beaconTime - getSimTime(node),
    ...
    BOOL* dropFrame)
{
    ...
    *dropFrame = FALSE;
    ...
    neighborItem->PER = e;
    neighborItem->lastLinkStateTime = getSimTime(node);
    ...
    BOOL* dropFrame)
{
    ...
    *dropFrame = TRUE;
    ...
    portalItem->lastPannTime = getSimTime(node);
            ...
            || mp->linkSetupRateLimit == 0))
    {
        ...
        if (neighborItem->state == DOT11s_NEIGHBOR_CANDIDATE_PEER)
        {
            ...
            if (neighborItem->lastBeaconTime >
                getSimTime(node) - DOT11s_LINK_SETUP_TIMER)
            {
    ...
    NetworkType networkType)
{
        ...
        || PHY_GetModel(node, dot11->myMacData->phyNumber) == PHY_ABSTRACT)
    {
        ...
        || PHY_GetModel(node, dot11->myMacData->phyNumber) == PHY_CHANSWITCH)
    {
        mp->initValues->beaconDuration =
