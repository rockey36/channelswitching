...
int MacDot11StationGetCurrentMessageSize(
    MacDataDot11* dot11)
{
    return (MESSAGE_ReturnPacketSize(MacDot11StationGetCurrentMessage(dot11)));
}// MacDot11StationGetCurrentMessage
           ...
           DOT11_SignalMeasurementInfo* meanMeas)
{
    ...
    oldestTimeStamp = getSimTime(node);
    for (i = 0; i < DOT11_MAX_NUM_MEASUREMNT; i ++)
    {
    ...
    measInfo[oldestMeas].measTime = getSimTime(node);
    ...
    meanMeas->measTime = getSimTime(node);
}
        ...
        const Message* const msg)
{
    ...
    DOT11_SignalMeasurementInfo meanMeas;
    int infoFieldSize = MESSAGE_ReturnInfoSize(newmsg);
    ...
    if(apInfo != NULL)
    {
        ...
        apInfo->lastMeaTime = getSimTime(node);
    }
}
...
//--------------------HCCA-Updates Start---------------------------------//
    if(dot11->state == DOT11e_CAP_START )
    {
        ...
        {
            fHdr->qoSControl.TXOP = (unsigned char)
                (dot11->hcVars->txopDurationGranted/ MILLI_SECOND);

            fHdr->qoSControl.TID = (unsigned short)dot11->hcVars->currentTSID;
    ...
    {
        fHdr->qoSControl.TID = (unsigned short)dot11->ACs[acIndex].priority;
        fHdr->qoSControl.EOSP = 1;
        fHdr->qoSControl.TXOP = (unsigned char)
...
BOOL MacDot11StationWaitForNAV(
    ...
    MacDataDot11* dot11)
{
    return (dot11->NAV > getSimTime(node));
...
{
    dot11->lastBOTimeStamp = getSimTime(node);
...
clocktype backoff = (getSimTime(node) - dot11->lastBOTimeStamp);
    ...
    if (!dot11->useDvcs) {
        ...
        if (((haveDirection) &&
             (DirectionalNav_SignalIsNaved(
                 ...
                 directionAngleToSend, getSimTime(node))))
                  ||
            ((!haveDirection) &&
             (!DirectionalNav_ThereAreNoNavs(
                 &dot11->directionalInfo->directionalNav, getSimTime(node)))))
        {
            // Should disregard NAV in Extended Ifs Mode. section 9.2.3.4
            if (!dot11->IsInExtendedIfsMode){
                mustWaitForNAV = TRUE;
...
//---------------------DOT11e--Updates------------------------------------//
            ...
            if (MacDot11IsQoSEnabled(node, dot11))
            {
                if (dot11->currentACIndex != DOT11e_INVALID_AC)
                {
                    clocktype extendedIfsDelay = dot11->sifs +
                    PHY_GetTransmissionDuration(
                        node, dot11->myMacData->phyNumber,
    ...
    Mac802Address destAddr)
{
    ...
    if (!entry) {
        ...
        entry->dataRateTimer = getSimTime(node) + dot11->rateAdjustmentTime;
        entry->next = NULL;
    ...
    MacDataDot11* dot11)
{
    if ( (dot11->dataRateInfo->ipAddress != ANY_MAC802 &&
          dot11->dataRateInfo->dataRateTimer < getSimTime(node)) ||
          ...
          DOT11_NUM_ACKS_FOR_RATE_INCREASE) )
    {
        ...
        dot11->dataRateInfo->dataRateTimer =
            getSimTime(node) + dot11->rateAdjustmentTime;
    }
    ...
    BOOL* nodeIsInCache)
{
    AngleOfArrivalCache_Update(
        ...
        nodeIsInCache,
        (getSimTime(node) + dot11->directionalInfo->defaultCacheExpirationDelay),
        getSimTime(node));
...
//---------------------------Power-Save-Mode-Updates---------------------//
    if(MacDot11IsATIMDurationActive(dot11)
        ...
        DOT11_AttachedNodeList* attachNodeList = dot11->attachNodeList;
             MESSAGE_Free(node,dot11->currentMessage);
             dot11->currentMessage = NULL;
             if(dot11->dot11TxFrameInfo!= NULL && dot11->dot11TxFrameInfo->frameType ==
                 DOT11_ATIM)
             {
                 MEM_free(dot11->dot11TxFrameInfo);
             }
    ...
    const Message* thePacketIfItGetsThrough)
{
    DOT11_ShortControlFrame* hdr = (DOT11_ShortControlFrame*)
        MESSAGE_ReturnPacket(thePacketIfItGetsThrough);
    ...
    if (!MAC_InterfaceIsEnabled(node, dot11->myMacData->interfaceIndex))
    {
     ...
     if (DEBUG) {
        ...
        printf("[%d] Sending to %20s (size %d) at %s %s state:%d\n",
               node->nodeId,
               macAdder, MESSAGE_ReturnPacketSize(packet),
               timeStr, delayStr,
               dot11->state);
    }
...
//---------------------------------------------------------------
 static
        ...
        MacDataDot11* dot11)
  {
        ...
        {
            canTransmit = MacDot11CanTransmitBeacon(node,dot11,sizeof(beacon));
           if( canTransmit)
           {
    ...
    MacDataDot11* dot11)
{
    ...
    if (MacDot11StationGetCurrentMessage(dot11) != NULL) {

         if (MacDot11StationGetCurrentMessageSize(dot11)
            <= DOT11_FRAG_THRESH)
         {
//---------------------DOT11e--Updates------------------------------------//
            ...
            if (MacDot11IsQoSEnabled(node, dot11))
            {
             packetTransmissionDuration =
                PHY_GetTransmissionDuration(
                    node, dot11->myMacData->phyNumber,
                    dot11->dataRateInfo->dataRateType,
                    MESSAGE_ReturnPacketSize(MacDot11StationGetCurrentMessage(dot11)));
    ...
    Mac802Address destAddr)
{
    ...
    PHY_SetTxDataRateType(node, dot11->myMacData->phyNumber, dataRateType);
