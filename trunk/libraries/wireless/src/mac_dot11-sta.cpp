...
//--------------------------------------------------------------------------
static // inline//
BOOL MacDot11StartTimerForNextAwakePeriod(
    Node* node,
    MacDataDot11* dot11)
{
    ...
    if(dot11->nextAwakeTime > getSimTime(node)){
        delay = dot11->nextAwakeTime - getSimTime(node);
    }
    else{
        ...
        delay = delay - DOT11_TIME_UNIT -
            (getSimTime(node) - dot11->beaconExpirationTime);
        dot11->nextAwakeTime = getSimTime(node) + delay;
    }// end of if - else
    if(delay <= 0){
        return FALSE;
    }
    ...
    if (MacDot11IsAp(dot11))
    {
        ...
        if (MacDot11IsAssociationDynamic(dot11)){
            if (dot11->beaconsMissed > DOT11_BEACON_MISSED_MAX){
                ...
                if (dot11->beaconTime < getSimTime(node))
   ...
   if(Dot11_GetCfsetFromBeacon(node, dot11, msg, &cfSet))
    {
      ...
      dot11->cfpRepetionInterval = ( (dot11->beaconInterval)
       ...
       beaconTransmitTime = getSimTime(node) - PHY_GetTransmissionDuration(
                                        node, dot11->myMacData->phyNumber,
                                        dataRateType,
