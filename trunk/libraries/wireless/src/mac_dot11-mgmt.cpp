    ...
    unsigned int tsid)
{
    ...
    newFrameInfo->insertTime = getSimTime(node);
...
//---------------------DOT11e--Updates------------------------------------//
        ...
        memset(assocFrame, 0, sizeof(DOT11e_AssocReqFrame));
...
//---------------------------Power-Save-Mode-End-Updates-----------------//
        strcpy(assocFrame->ssid, dot11->stationMIB->dot11DesiredSSID);      // String copy over the BSSID
    ...
    if (MacDot11IsAp(dot11)) {  // ACCESS POINT
        ...
        if (!MacDot11ApStationListIsEmpty(node, dot11, ap->apStationList))
        {
            ...
            if(stationItem != NULL)
            {
                ...
                stationItem->data->LastFrameReceivedTime = getSimTime(node);
            }
        }
            if(stationItem == NULL)
            {
...
//---------------------DOT11e--Updates------------------------------------//
// /**
// FUNCTION   :: MacDot11eManagementBuildAssociateResponseFrame
// LAYER      :: MAC
// PURPOSE    :: Build an associate response frame from an ESS
// PARAMETERS ::
// + node           : Node* :Node Pointer
// + dot11          : MacDataDot11* :  Pointer to Dot11 structure
// + param frame    :  DOT11_Frame*    : Pointer to a new empty frame
// + param reqFrame : DOT11e_AssocReqFrame*    : Received the request frame
// RETURN    :: int : 0 or 1
// **/
...
//---------------------DOT11e--Updates------------------------------------//
// /**
// FUNCTION   :: MacDot11eManagementBuildReassociateResponseFrame
// LAYER      :: MAC
// PURPOSE    :: Build an associate response frame from an ESS
// PARAMETERS ::
// + node           : Node* :Node Pointer
    ...
    DOT11_ProbeReqFrame* reqFrame)
{
    ...
    if (MacDot11IsAp(dot11)) {  // ACCESS POINT
        ...
        probe->beaconInterval = (unsigned short)dot11->beaconInterval;
        ...
        probe->timeStamp = getSimTime(node) + dot11->delayUntilSignalAirborn;
        DOT11_FrameInfo* newFrameInfo =
        (DOT11_FrameInfo*) MEM_malloc(sizeof(DOT11_FrameInfo));
...
//---------------------DOT11e--Updates------------------------------------//
        ...
        memset(reassocFrame, 0, sizeof(DOT11e_ReassocReqFrame*));
        ...
        *frameInfo = newframeInfo;
        ...
        //assocFrame->supportedRates = NULL;   // Add supported rates here
...
//--------------------DOT11e-End-Updates---------------------------------//
        ...
        // Fill in frame
        //assocFrame->capInfo = NULL;          // Add capability info here
//---------------------------Power-Save-Mode-Updates---------------------//
        // Add listen interval information in association frame request frame
...
//---------------------------Power-Save-Mode-End-Updates-----------------//
        strcpy(reassocFrame->ssid, dot11->stationMIB->dot11DesiredSSID);      // String copy over the BSSID
        //assocFrame->supportedRates = NULL;   // Add supported rates here
        DOT11_FrameInfo* newframeInfo =
        (DOT11_FrameInfo*) MEM_malloc(sizeof(DOT11_FrameInfo));
            ...
            mngmtVars->state < DOT11_S_M_JOINED)
        {
            ...
            if(mngmtVars->apAddr == hdr->sourceAddr)
            {
                if(dot11->pcfEnable)
                {
                    if (cfSet.cfpCount == 0 && timFrame.dTIMCount == 0){
                       if (newNAV > dot11->NAV) {
                           //MacDot11CfpResetBackoffAndAckVariables(node,
                             //                                    dot11);
                        dot11->NAV = newNAV;
                       }
    ...
    DOT11_Frame* rxFrame)
{
 ...
 //---------------------DOT11e--Updates------------------------------------//
    if (MacDot11IsQoSEnabled(node, dot11) && isQSTA)
    {
       ...
       // Get the request frame
       DOT11e_AssocReqFrame * respFrame =
       (DOT11e_AssocReqFrame *) MESSAGE_ReturnPacket(rxFrame);
...
{
    int result = DOT11_R_FAILED;
    printf("node %d: MacDot11ManagementProcessProbeRequest \n", node->nodeId);
    ...
    DOT11_Frame* rxFrame)
{
    ...
    // dot11s. Currently only an MAP processes this frame.
    //ERROR_Assert(dot11->isMAP,
      //  "MacDot11ManagementProcessReassociateRequest: "
      //  "Reassociation limited to Mesh APs.\n");
    ...
    TrafficSpec* TSPEC)
{
    BOOL isFound = FALSE;
    DOT11_FrameHdr* frameHdr = (DOT11_FrameHdr*) MESSAGE_ReturnPacket(msg);
    //ERROR_Assert(frameHdr->frameType == DOT11_PROBE_REQ,
      //  "Dot11_GetTspecFromFrame: Wrong frame type.\n");
    ...
    DOT11_Frame* rxFrame)
{
    ...
    if(MacDot11IsAp(dot11))
    {
        // printf("AP node %d - got a management frame \n", node->nodeId);
    ...
    switch (hdr->frameType) {
        ...
        case DOT11_PROBE_REQ:
        {
            // printf("node %d: case DOT11_PROBE_REQ \n", node->nodeId);
            dot11->mgmtSendResponse = TRUE;
            if(MacDot11IsAp(dot11))
            {
    ...
    int result)
{
    ...
    if(scanStatus == DOT11_R_FAILED && dot11->stationAssociated)
    {
        ...
        //if(signal strength of the AP is less)
        //{
        //dissociate
        //}
    }
    ...
    MacDataDot11* dot11)
{
    ...
    if (MacDot11IsAp(dot11)) {  // ACCESS POINT
        ...
        if ( mngmtVars->scanType == DOT11_SCAN_DISABLED ) {
            ...
            ERROR_Assert(mngmtVars->currentChannel > 0,
            ...
            mngmtVars->channelInfo->scanStartTime = getSimTime(node);
        ...
        {
            printf("node %d, bss %d: MacDot11ManagementScanStart active scan start\n", node->nodeId, dot11->bssAddr);

            //bss shouldn't change for ad-hoc chanswitch nodes
            if(dot11->chanswitchType != DOT11_CHANSWITCH_TYPE_AP_PROBE){
                dot11->bssAddr = dot11->selfAddr;
            }
             // Set first channel
    ...
    Message* msg)
{
    ...
    switch(msg->eventType)
    {
        ...
        case MSG_MAC_DOT11_ChanswitchRequest:
        {
            ...
            "MSG_MAC_DOT11_ChanswitchRequest: Channel switching type must be set to AP Probing. \n");   
            // printf("Received request to StartJoin from CHANSWITCH node %d \n", node->nodeId);
            ...
            case MSG_MAC_DOT11_ChangeChannelRequest:
        {
            ...
            //send message back to app layer
            break;
        }
    ...
    MacDataDot11* dot11)
{
    ...
    if (MacDot11IsAp(dot11)) {  // ACCESS POINT
        ...
        printf("node %d: MacDot11ManagementEnable for AP \n", node->nodeId);
    ...
    NetworkType networkType)
{
    ...
    mngmtVars->printManagementStatistics = TRUE;
    ...
    if (wasFound) {
        mngmtVars->currentChannel = (unsigned)retInt;
        ERROR_Assert(retInt >= 0 ,
            "MacDot11Init: "
            "Value of MAC-DOT11-STA-CHANNEL is negative.\n");

        if (mngmtVars->currentChannel >=
                     (unsigned int)PROP_NumberChannels(node))
        {
            ERROR_ReportWarning("MacDot11Init: "
        ...
        else if (!PHY_CanListenToChannel(node,
                        dot11->myMacData->phyNumber,
                        mngmtVars->currentChannel))
        {
            ERROR_ReportWarning("MacDot11Init: "
                "Cannot listen to configured value of MAC-DOT11-STA-CHANNEL "
                "Setting the channel to first listenable channel");
            mngmtVars->currentChannel = firstListeningChannel;
        }
    }
    else {
