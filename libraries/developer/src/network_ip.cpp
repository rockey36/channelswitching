...
#define IPV4_ROUTING_DISABLED_WARNING 1
...
// End for Dymo
    ...
    if (retVal)
    {
         ERROR_Assert(ipFragUnit >= 64 && ipFragUnit <= 65535,
             "Fragmentation unit should be greater than 64 "
    ...
    else
    {
        if (fragHead)
        {
         ...
         // Below is the same code that used for non-fragment message above
                ...
                // Handle packet to security protocol if IpSec enable
                // over this interface
                if (outgoingInterface != CPU_INTERFACE
                    && ip->interfaceInfo[outgoingInterface]->spdOUT
...
//GuiStart
    if ((node->guiOption == TRUE) && ipHeader->ip_p != IPPROTO_CELLULAR)
    {
...
//GuiEnd
    ...
    //as per rfc 791, the TTL of a packet MUST NOT be checked 
                        ...
                        NodeAddress previousHopAddress)
{
    ...
    if (incomingInterface == CPU_INTERFACE)
    {
        ...
        if (!IsIPsecProcessed(node, msg)
            ...
            && ipHeader->ip_dst != ANY_DEST
            && (ipHeader->ip_dst != NetworkIpGetInterfaceBroadcastAddress(node, outgoingInterface))
            && !NetworkIpIsMulticastAddress(node, ipHeader->ip_dst)
...
NetworkIpInitStats(Node* node, NetworkIpStatsType *stats)
{
    ...
    if (node->guiOption) {
        stats->ipInReceivesId = GUI_DefineMetric("IP: Number of Packets Received",
                                                 node->nodeId,
                                                 GUI_NETWORK_LAYER, 0,
                                                 GUI_UNSIGNED_TYPE,
                                                 GUI_CUMULATIVE_METRIC);

        stats->ipInHdrErrorsId = GUI_DefineMetric("IP: Number of Packets Received with Errors in IP Header",
                                                  node->nodeId,
                                                  GUI_NETWORK_LAYER, 0,
                                                  GUI_UNSIGNED_TYPE,
                                                  GUI_CUMULATIVE_METRIC);
    ...
    BOOL fragmentForMpls)
{
    ...
    IpHeaderSetIpFragOffset(&(ipHeader->ipFragment), origFragOffSet);
#ifdef ADDON_BOEINGFCS
        ...
        off < (packetLen - hlen); off += fragmentedLen)
    {
        ...
        IpHeaderSetIpLength(&(ipHeader->ip_v_hl_tos_len), curPayloadLen + hlen);
    ...
    for (i = 0; i < node->numberInterfaces; i++)
    {
            ...
            || NetworkIpGetInterfaceType(node, i) == NETWORK_DUAL)
        {
            ...
            if (retVal)
            {
                ...
                {
                    ERROR_ReportMissingLibrary(protocolString, "NETSEC_LIB");
                }
#endif // NETSEC_LIB
