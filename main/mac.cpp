...
#include "mac_stats.h"
#include "mac_boeingfcs_usap.h"
#include "mac_ces_usap.h"
#include "mac_abstract.h"
#include "mac_ces_wintncw.h"
#include "mac_ces_winthnw.h"
#include "mac_ces_wintgbs.h"
#include "boeing_genericmac.h"
#include "mac_ces_srw.h"
#include "phy_stats.h"
#include "ces2.h"
...
// **/
inline NodeId GetNodeIdFromMacAddress (unsigned char byte[])
{
...
//#endif
        {
...
#ifdef ADDON_BOEINGFCS
                strcmp(macProtocolName, "MAC-CES-USAP") == 0 ||
                ...
                strcmp(macProtocolName, "MAC-CES-SRW") == 0 ||
                ...
                Address* address,const NodeInput *nodeInput)
{
    ...
    if (!node->disabled)
    {
        node->macData[interfaceIndex]->resetFunctionList = new MacResetFunctionList;
        node->macData[interfaceIndex]->resetFunctionList->first = NULL;
        node->macData[interfaceIndex]->resetFunctionList->last = NULL;
    }
#endif
}
    ...
    short       subnetIndex)
{
    ...
    else if (strcmp(macProtocolName, "ANE") == 0)
    {
        ...
        {
            char errorStr[MAX_STRING_LENGTH];
            sprintf(errorStr, "Failure to initialized SatTsm Module subnet %d.",
                subnetAddress);
            ERROR_ReportError(errorStr);
        }
    ...
    if (strncmp(macProtocolName, "FCSC-", 5) == 0) {
        ...
        if (strncmp(phyModelName, "FCSC-", 5) == 0) {
            ...
            ERROR_ReportMissingLibrary(phyModelName, "Cellular");
             ...
             PHY_CreateAPhyForMac(
                     ...
                     &node->macData[interfaceIndex]->phyNumber);
 #else // UMTS_LIB
             ERROR_ReportMissingLibrary(phyModelName, "UMTS");
 #endif // UMTS_LIB
         }
        ...
        {
#ifdef MILITARY_RADIOS_LIB
            printf("AddNodeToSubnet(): PHY-JTRS-WNW\n");
            PHY_CreateAPhyForMac(
                    node,
                    nodeInput,
                    interfaceIndex,
            ...
            ERROR_ReportMissingLibrary(phyModelName, "Military Radios");
#endif // MILITARY_RADIOS_LIB
        }
        ...
        {
#ifdef MILITARY_RADIOS_LIB
           node->macData[interfaceIndex]->macProtocol = MAC_PROTOCOL_USAP;
           MacUsapInit(node, interfaceIndex, interfaceAddress,
...
#ifdef ADDON_BOEINGFCS
        else if (strcmp(macProtocolName, "MAC-CES-USAP") == 0) {
            node->macData[interfaceIndex]->macProtocol = MAC_PROTOCOL_CES_USAP;
            MacBoeingFcsUsapInit(node, interfaceIndex, nodeInput, subnetListIndex, nodesInSubnet);
#ifdef ADDON_NGCNMS
            //set MacCesUsapReset and MacBoeingFcsUsapReset to be call at node-fault
            // or when there is a change in node ip-address
            if (!node->enabled)
            {
                MAC_AddResetFunctionList(node, interfaceIndex, (void *)MacCesUsapReset);
            }
#endif
        }
            ...
            MacCesWintHnwInit(node, interfaceIndex, interfaceAddress,
                       nodeInput, (SubnetMemberData *) subnetList, subnetListIndex, nodesInSubnet);
        }
        else
        if (strcmp(macProtocolName, "BOEING-GENERICMAC") == 0 ||
            strcmp(macProtocolName, "MAC-CES-EPLRS") == 0 ||
            strcmp(macProtocolName, "MAC-CES-SINCGARS") == 0)
        {
            node->macData[interfaceIndex]->macProtocol = MAC_PROTOCOL_BOEING_GENERICMAC;
            BoeingGenericMacInit(node, interfaceIndex, interfaceAddress,
                           nodeInput, nodesInSubnet);
        }
        else
        if (strcmp(macProtocolName, "MAC-CES-SRW") == 0) {
            node->macData[interfaceIndex]->macProtocol = MAC_PROTOCOL_CES_SRW;
            MacCesSrwInit(node, interfaceIndex, interfaceAddress,
                        nodeInput);
        }
#endif // ADDON_BOEINGFCS
...
#elif UMTS_LIB // UMTS_LIB
            node->macData[interfaceIndex]->macProtocol = MAC_PROTOCOL_CELLULAR;
            MacCellularInit(node,interfaceIndex, nodeInput);
#else
            ERROR_ReportMissingLibrary(macProtocolName, "Cellular/UMTS");
#endif // CELLULAR_LIB
        }
...
#ifdef MILITARY_RADIOS_LIB
            printf("AddNodeToIpv6Network(): PHY-JTRS-WNW\n");
            PHY_CreateAPhyForMac(
                    node,
                    nodeInput,
                    interfaceIndex,
...
#ifdef MILITARY_RADIOS_LIB
           node->macData[interfaceIndex]->macProtocol = MAC_PROTOCOL_USAP;
           MacUsapInit(node, interfaceIndex, linkLayerAddr,
                       nodeInput, nodesInSubnet, subnetListIndex,
                       0, 0);
#else // MILITARY_RADIOS_LIB
            ERROR_ReportMissingLibrary(macProtocolName, "Military Radios");
...
void MacInterfaceStatsInitialize (Node* node)
{
    while (node != NULL)
    {
            ...
            interfaceIndex++)
        {
            ...
            // Let's make the Mib variable dynamic
            ...
            if (h->IsEnabled())
            {
                ...
                if (createPath)
                {
                    ...
                    h->AddObject(
                        path,
                        new D_UInt32Obj(&node->macData[interfaceIndex]->ifOperStatus));

                    h->SetWriteable(path, FALSE);
                    h->SetExecutable(path, FALSE);
                    sprintf(mibsIdPath,
                        "/node/%d/snmp/1.3.6.1.2.1.2.2.1.8.%d",
                        node->nodeId,
                        interfaceIndex);
                    h->AddLink(mibsIdPath, path);

                    // AddLink for administrative status, same as Operational status.
                    sprintf(mibsIdPath,
                        "/node/%d/snmp/1.3.6.1.2.1.2.2.1.7.%d",
                        node->nodeId,
                        interfaceIndex);
                    h->AddLink(mibsIdPath, path);
                }

                // Create path for IfInOctets
                createPath =
                    h->CreateNodeInterfacePath(
                    node,
                    interfaceIndex,
                ...
                if (createPath)
                {
                    ...
                    h->AddObject(
                        path,
                        new D_UInt32Obj(&node->macData[interfaceIndex]->ifInOctets));

                    h->SetWriteable(path, FALSE);
                    h->SetExecutable(path, FALSE);
                    sprintf(mibsIdPath,
                        "node/%d/snmp/1.3.6.1.2.1.2.2.1.10.%d",
                        node->nodeId,
                        interfaceIndex);
                    h->AddLink(mibsIdPath, path);
                }
                // Create path for IfOutOctets
                createPath =
                    h->CreateNodeInterfacePath(
                    node,
                    interfaceIndex,
                ...
                if (createPath)
                {
                    ...
                    h->AddObject(
                        path,
                        new D_UInt32Obj(&node->macData[interfaceIndex]->ifOutOctets));

                    h->SetWriteable(path, FALSE);
                    h->SetExecutable(path, FALSE);
                    sprintf(mibsIdPath,
                        "node/%d/snmp/1.3.6.1.2.1.2.2.1.16.%d",
                        node->nodeId,
                        interfaceIndex);
                    h->AddLink(mibsIdPath, path);
                }
                // Create path for IfInErrors
                createPath =
                    h->CreateNodeInterfacePath(
                    node,
                    interfaceIndex,
                ...
                if (createPath)
                {
                    ...
                    h->AddObject(
                        path,
                        new D_UInt32Obj(&node->macData[interfaceIndex]->ifInErrors));

                    h->SetWriteable(path, FALSE);
                    h->SetExecutable(path, FALSE);
                    sprintf(mibsIdPath,
                        "node/%d/snmp/1.3.6.1.2.1.2.2.1.14.%d",
                        node->nodeId,
                        interfaceIndex);
                    h->AddLink(mibsIdPath, path);
                }
                // Create path for IfOutErrors
                createPath =
                    h->CreateNodeInterfacePath(
                    node,
                    interfaceIndex,
                ...
                if (createPath)
                {
                    ...
                    h->AddObject(
                        path,
                        new D_UInt32Obj(&node->macData[interfaceIndex]->ifOutErrors));

                    h->SetWriteable(path, FALSE);
                    h->SetExecutable(path, FALSE);
                    sprintf(mibsIdPath,
                        "node/%d/snmp/1.3.6.1.2.1.2.2.1.20.%d",
                        node->nodeId,
                        interfaceIndex);
                    h->AddLink(mibsIdPath, path);
                }
                // Create path for IfHCInOctets
                createPath =
                    h->CreateNodeInterfacePath(
                    node,
                    interfaceIndex,
                ...
                if (createPath)
                {
                    ...
                    h->AddObject(
                        path,
                        new D_UInt64Obj(&node->macData[interfaceIndex]->ifHCInOctets));

                    h->SetWriteable(path, FALSE);
                    h->SetExecutable(path, FALSE);
                    sprintf(mibsIdPath,
                        "node/%d/snmp/1.3.6.1.2.1.31.1.1.1.6.%d",
                        node->nodeId,
                        interfaceIndex);
                    h->AddLink(mibsIdPath, path);
                }
                // Create path for IfHCOutOctets
                createPath =
                    h->CreateNodeInterfacePath(
                    node,
                    interfaceIndex,
                ...
                if (createPath)
                {
                    ...
                    h->AddObject(
                        path,
                        new D_UInt64Obj(&node->macData[interfaceIndex]->ifHCOutOctets));

                    h->SetWriteable(path, FALSE);
                    h->SetExecutable(path, FALSE);
                    sprintf(mibsIdPath,
                        "node/%d/snmp/1.3.6.1.2.1.31.1.1.1.10.%d",
                        node->nodeId,
                        interfaceIndex);
                    h->AddLink(mibsIdPath, path);
                }
            ...
            nodeIter++)
        {
                ...
                interfaceIndex++)
            {
                ...
                if (createPath)
                {
                    ...
                    std::string     macTypeByIndexPath;
                    h->BuildNodeInterfaceIndexPath (
                        node, interfaceIndex,
                        "macType",
                        macTypeByIndexPath);
                    h->AddLink(macTypeByIndexPath, path);
                }
            }
        }
    }
    ...
    unsigned short& siteCounter)
{
    ...
    unsigned int subnetPrefixLen1;
    unsigned int subnetPrefixLen2;
                  ...
                  unsigned short& siteCounter)
{
    ...
    {
        NodeAddress interfaceAddress = ANY_ADDRESS;
        NodeAddress subnetAddress;
        ...
        if (subnetList[count].node != NULL)
        {
           ...
           {
#ifdef ADDON_NGCNMS
                /* JDL -- commented out, not parallel safe
                BOOL isAbstract = FALSE;
                isAbstract = subnetAbstractIsAbstractNode(
                    node,
                    subnetList[count].address.interfaceAddr.ipv4,
                    nodeInput);
                if (isAbstract) {
                    sprintf(macProtocolName, "CSMA");
                } */
#endif
                                 ...
                                 int   interfaceIndex)
{
    ...
    if (node->macData[interfaceIndex]->faultCount > 1)
    {
        return;
    }
#ifdef ADDON_BOEINGFCS
    ...
    {
        char time[20];
        TIME_PrintClockInSecond(getSimTime(node), time, node);
...
MAC_ProcessEvent(Node *node, Message *msg)
{
    ...
    if (MESSAGE_GetEvent(msg) == MSG_MAC_StartFault)
    {
        ...
        {
            MacStatsStartTimerDown(node, interfaceIndex);
#endif
        }

        MAC_ProcessDisableInterface(node, interfaceIndex);
    ...
    else if (MESSAGE_GetEvent(msg) == MSG_MAC_EndFault)
    {
        ...
        if (node->macData[interfaceIndex]->macProtocol ==
            MAC_PROTOCOL_CES_USAP)
        {
            MacStatsUpdateTimeDown(node, interfaceIndex);
        }
#endif
...
#endif // ENTERPRISE_LIB
#ifdef ADDON_BOEINGFCS
#if MAC_ABSTRACT_ENABLED
    if (protocol == MAC_PROTOCOL_ABSTRACT)
    {
        MacAbstractLayer(node, interfaceIndex, msg);
        return;
    }
#endif /* MAC_ABSTRACT_ENABLED */
#endif /* ADDON_BOEINGFCS */
    ...
    switch (node->macData[interfaceIndex]->macProtocol)
    {
        ...
        case MAC_PROTOCOL_USAP:
        {
            MacUsapLayer(node, interfaceIndex, msg);
            break;
        }
#endif // MILITARY_RADIOS_LIB
#ifdef ADDON_BOEINGFCS
     case MAC_PROTOCOL_CES_USAP:
     {
         MacBoeingFcsUsapLayer(node, interfaceIndex, msg);
         break;
     }
     case MAC_PROTOCOL_CES_WINTNCW:
     {
     ...
     case MAC_PROTOCOL_BOEING_GENERICMAC:
     {
          BoeingGenericMacProcessEvent(node, interfaceIndex, msg);
          break;
     }
     case MAC_PROTOCOL_CES_SRW:
     {
          MacCesSrwLayer(node, interfaceIndex, msg);
          break;
     }
#endif // ADDON_BOEINGFCS
...
MAC_NetworkLayerHasPacketToSend(Node *node, int interfaceIndex)
{
    ...
    if (!MAC_InterfaceIsEnabled(node, interfaceIndex))
    {
        ...
        {
            nextHopAddress = MacCesSincgarsOutputQueueDropPacket(node,
                                                           interfaceIndex,
                                                           &msg);
    ...
    switch (node->macData[interfaceIndex]->macProtocol)
    {
        ...
        case MAC_PROTOCOL_USAP:
        {
            MacUsapNetworkLayerHasPacketToSend(
                node,
                (MacDataUsap *) node->macData[interfaceIndex]->macVar);
            break;
        }
#endif // MILITARY_RADIOS_LIB
        ...
        case MAC_PROTOCOL_CELLULAR:
        {
            MacCellularNetworkLayerHasPacketToSend(node, interfaceIndex, (MacCellularData *) node->macData[interfaceIndex]->macVar);
            break;
        }
#elif UMTS_LIB
        case MAC_PROTOCOL_CELLULAR:
...
#undef PARTICIPANT
        ...
        case MAC_PROTOCOL_CES_USAP:
        {
            MacBoeingFcsUsapNetworkLayerHasPacketToSend(
                 node,
                 (MacCesDataUsap*) node->macData[interfaceIndex]->macVar);
            break;
        }
        case MAC_PROTOCOL_CES_WINTNCW:
        {
        ...
        case MAC_PROTOCOL_BOEING_GENERICMAC:
        {
            //j.h - if EPLRS is enabled, queue packets in needline queue according to priorities
            if (MacCesEplrsActiveOnInterface(node, interfaceIndex))
            {
                MacCesEplrsIncQueuePacketFromNetworkLayer(node, interfaceIndex);
            }
            else
            {
                MacBoeingGenericMacNetworkLayerHasPacketToSend(
                    node,
                   (BoeingGenericMacData *) node->macData[interfaceIndex]->macVar);
            }
            break;
        }
        case MAC_PROTOCOL_CES_SRW:
        {
            MacCesSrwNetworkLayerHasPacketToSend(
                    node, (MacCesSrwData*) node->macData[interfaceIndex]->macVar);
            break;
        }
#endif // ADDON_BOEINGFCS
    ...
    Message *packet)
{
...
#ifdef ADDON_BOEINGFCS
    // FOR MIBS: IfInOctets.
    node->macData[interfaceIndex]->ifInOctets += MESSAGE_ReturnPacketSize(packet);
    node->macData[interfaceIndex]->ifHCInOctets += MESSAGE_ReturnPacketSize(packet);
#endif
    ...
    switch (node->macData[interfaceIndex]->macProtocol)
    {
     ...
     case MAC_PROTOCOL_CES_USAP:
     {
         MacBoeingFcsUsapReceivePacketFromPhy(
             node,
             (MacCesDataUsap*)node->macData[interfaceIndex]->macVar,
             packet);
         break;
     ...
     case MAC_PROTOCOL_BOEING_GENERICMAC:
     {
          MacBoeingGenericMacReceivePacketFromPhy(
              node,
              (BoeingGenericMacData *) node->macData[interfaceIndex]->macVar,
              packet);
          break;
    ...
    int numError)
{
    ...
    switch (node->macData[interfaceIndex]->macProtocol)
    {
        case MAC_PROTOCOL_SPAWAR_LINK16:
        {
            MacSpawar_Link16ReceivePacketFromPhy(
                node,
...
{
    assert(req != 0 && resp != 0); // references...one day

    switch (node->macData[interfaceIndex]->macProtocol) {
#ifdef ADDON_BOEINGFCS
        case MAC_PROTOCOL_CES_USAP:
            MacBoeingFcsUsapManagementRequest(
                node,
                interfaceIndex,
                req,
                resp);
    ...
    const Message *potentialIncomingPacket)
{
    switch (node->macData[interfaceIndex]->macProtocol)
    {
        ...
        case MAC_PROTOCOL_USAP:
        {
            MacUsapReceivePhyStatusChangeNotification(
                node,
                (MacDataUsap*)node->macData[interfaceIndex]->macVar,
                oldPhyStatus,
                newPhyStatus);
     ...
     case MAC_PROTOCOL_CES_USAP:
     {
         MacBoeingFcsUsapReceivePhyStatusChangeNotification(
             node,
             (MacCesDataUsap*)node->macData[interfaceIndex]->macVar,
             oldPhyStatus,
             newPhyStatus);
     ...
     case MAC_PROTOCOL_BOEING_GENERICMAC:
     {
          MacBoeingGenericMacReceivePhyStatusChangeNotification(
              node,
              (BoeingGenericMacData *) node->macData[interfaceIndex]->macVar,
              oldPhyStatus,
              newPhyStatus);
     ...
     case MAC_PROTOCOL_CES_SRW:
     {
         MacCesSrwReceivePhyStatusChangeNotification(
             node,
             (MacCesSrwData*)node->macData[interfaceIndex]->macVar,
             oldPhyStatus,
             newPhyStatus);
         break;
     }
...
void MAC_RunTimeStat(Node *node)
{
    ...
    case MAC_PROTOCOL_BOEING_GENERICMAC:
    {
        BoeingGenericMacRunTimeStat(
                    node,
                    (BoeingGenericMacData *) node->
                    macData[interfaceIndex]->macVar);
    }
#endif // ADDON_BOEINGFCS
...
MAC_Finalize(Node *node)
{
         ...
         interfaceIndex++)
    {
     ...
     case MAC_PROTOCOL_CES_USAP:
     {
         MacBoeingFcsUsapFinalize(node, interfaceIndex);
         break;
     }
     case MAC_PROTOCOL_CES_WINTNCW:
     {
    ...
    case MAC_PROTOCOL_BOEING_GENERICMAC:
    {
        BoeingGenericMacFinalize(node, interfaceIndex);
        break;
    }
    case MAC_PROTOCOL_CES_SRW:
    {
        MacCesSrwFinalize(node, interfaceIndex);
        break;
    }
#endif // ADDON_BOEINGFCS
#ifdef MILITARY_RADIOS_LIB
                ...
                case MAC_PROTOCOL_USAP:
                {
                    MacUsapFinalize(node, interfaceIndex);
                    break;
                }
#endif //MILITARY_RADIOS_LIB
#ifdef ENTERPRISE_LIB
...
//if running BOEINGFCS and ABSTRACTION is enabled, allow MAC_ABSTRACT finalize
#ifdef ADDON_BOEINGFCS
#if MAC_ABSTRACT_ENABLED
     if (node->macData[interfaceIndex]->macProtocol == MAC_PROTOCOL_CES_USAP)
     {
        MacAbstractFinalize(node, interfaceIndex);
     }
#endif /* MAC_ABSTRACT_ENABLED */
#endif // ADDON_BOEINGFCS
...
//    ARP Phase
    ...
    if (MacCesSincgarsActiveOnInterface(node, interfaceIndex))
    {
        return MacCesSincgarsOutputQueueIsEmpty(node, interfaceIndex);
    }
    else if (MacCesEplrsActiveOnInterface(node, interfaceIndex)) {
        return MacCesEplrsIncOutputQueueIsEmpty(node, interfaceIndex);
    }
    else
#endif

#ifdef MILITARY_RADIOS_LIB
    if (EplrsActiveOnInterface(node, interfaceIndex)) {
        return EplrsIncOutputQueueIsEmpty(node, interfaceIndex);
    }
    else
#endif // MILITARY_RADIOS_LIB
    ...
    int *networkType)
{
    ...
    {
        dequeued =
            MacCesSincgarsOutputQueueDequeueFMU(
                node, interfaceIndex, msg, &nextHopAddress);

        MAC_FourByteMacAddressToVariableHWAddress(node,interfaceIndex,
                                                  destHWAddr,nextHopAddress);
    }
    else if (MacCesEplrsActiveOnInterface(node, interfaceIndex)) { //j.h
        dequeued =
            MacCesEplrsIncOutputQueueDequeuePacketForAPriority(
                node, interfaceIndex, priority, msg, &nextHopAddress);
...
#ifdef MILITARY_RADIOS_LIB
    if (EplrsActiveOnInterface(node, interfaceIndex)) { //j.h
        dequeued =
            EplrsIncOutputQueueDequeuePacketForAPriority(
                                    node,
                                    interfaceIndex,
                                    priority,
                                    msg,
    ...
    TosType *priority)
{
    ...
    *networkType = NETWORK_PROTOCOL_IP;
    ...
    if (MacCesSincgarsActiveOnInterface(node, interfaceIndex)) {
        aPacket =
            MacCesSincgarsOutputQueueTopFMU(
                node, interfaceIndex, msg, &nextHopAddress);
        ...
        aPacket =
            MacCesEplrsIncOutputQueueTopPacket(
    ...
    if (EplrsActiveOnInterface(node, interfaceIndex)) { //j.h
        aPacket =
            EplrsIncOutputQueueTopPacket(node, interfaceIndex, msg,
                                       &nextHopAddress, destHWAddr,priority);
    }
    else
#endif // MILITARY_RADIOS_LIB
...
#ifdef ADDON_BOEINGFCS
    MacCesDataUsap *usap = (MacCesDataUsap*)node->macData[interfaceIndex]->macVar;

    int subnetListIndex = usap->subnetListIndex;
#endif

    node->macData[interfaceIndex]->macVar = NULL;

    int nodesInSubnet = MAPPING_GetNumNodesInSubnet(node, subnetAddress);
...
void D_Fault::ReadAsString(std::string& out)
{
    if (m_IsFault)
    {
        out = "yes";
